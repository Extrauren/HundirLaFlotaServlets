...
public class Partida {
	...
	private boolean[][] misDisparos = null;	//matriz de booleanos. Que casilla se han disparado
	...
	public int pruebaCasilla(int f, int c) {
		...
		//System.out.println(posicionBarco);    //PRINT PARA DEBUGEAR
		if(!casillaDisparada(f,c)) {	//usamos el metodo que hemos creado 
			misDisparos[f][c] = true;
		}

		if (posicionBarco == AGUA) return AGUA;

		//Aquí tratamos con los barcos
		if(posicionBarco > AGUA){
			Barco barco = barcos.get(posicionBarco);
			barco.tocaBarco();
			//si el tam coincide con tocadaos lo hundimos 
			if(barco.getTamanyo() == barco.getTocadas()) {
				...
				//desde la columna x a la fila y (hasta hundirlo)
				if (orientacion == 'V') {
					for(int i=0; i<tam; i++) {
				...
				//Barco vertical
				//en funcion del numero de casillas del barco lo marcaremos como hundido
				//desde la fila x a la colunmna y (hasta hundirlo)
				else{
					for(int i=0; i<tam; i++) {
	 ...
	 * @param	f	fila de la casilla
	 * @param   c   columna de la casilla
	 * @return		resultado de marcar la casilla: true si se ha disparado, false si no
	 */	
	public boolean casillaDisparada(int fila , int col) {
	...
	/**
	 * rellena matriz misDisparos a False
	 */
